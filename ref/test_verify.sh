...
# Build a canonical root fixture: mkdir DIR (and any extra subdirs),
# then copy the repo's README.md + SEED.md into DIR. Negative tests
# usually overwrite one of the two files with a malformed version
# after calling this — the canonical copy is idempotent.
make_root_fixture() {
  local dir=$1; shift
  mkdir -p "$dir" "$@"
  cp "$here/README.md" "$dir/README.md"
  cp "$here/SEED.md"   "$dir/SEED.md"
}
...
make_root_fixture "$tmp/bad-readme"
...
make_root_fixture "$tmp/bad-normative"
...
# re-declare).
make_root_fixture "$tmp/has space" "$tmp/has space/sub dir"
...
make_root_fixture "$tmp/bad-purpose"
awk '
  BEGIN { swap=0 }
...
make_root_fixture "$tmp/bad-child" "$tmp/bad-child/sub dir"
...
make_root_fixture "$tmp/missing-readme" "$tmp/missing-readme/orphan"
...
make_root_fixture "$tmp/bad-descendant" "$tmp/bad-descendant/child"
...
make_root_fixture "$tmp/bad-cousin" "$tmp/bad-cousin/sibling" "$tmp/bad-cousin/branch"
...
make_root_fixture "$tmp/bad-skip-readme" "$tmp/bad-skip-readme/close/sub"
...
make_root_fixture "$tmp/bad-nested-normative" "$tmp/bad-nested-normative/sub"
