...
1. **Clone mode** — arg matches `https://...` or `git@host:...` (per SEED.md's supported transports). Ask the user once where to clone; default to `$HOME/Hacking/<repo>/`, falling back to `<default>2`, `<default>3`, etc. if the default already exists. Display and confirm the clone command before running: `git clone -- <url-for-display> <target>` (the `--` prevents a URL starting with `-` from being parsed as a flag). If the URL contains userinfo, query, or fragment (`https://user:token@host/path?ref=...#frag`), strip all three from the **displayed** command — never show credentials in the confirmation transcript. Pass the original (un-redacted) URL only as the argv argument to `git clone`, so the clone still authenticates. Same redaction rule as the feedback `seed_url` payload field; see [[../../../SEED#^act-feedback]].
